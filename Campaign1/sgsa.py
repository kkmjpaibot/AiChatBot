...
def calculate_premium_estimation(annual_income, years_of_coverage, age):
    ...
    if age <= 30:
        ...
        premium_rate_per_thousand = 4.80
...
Would you like to get a personalized quote?"""
    ...
    async def _handle_premium_decision(self, user_id: str, message: Union[str, dict], state: CampaignState) -> Dict[str, Any]:
        ...
        try:
            ...
            if (button_value and button_value.lower() in positive_responses) or \
               (text_input and text_input.lower() in positive_responses):
                # Age restriction check after "get quote"
                age = None
                # Try to get age from user_data in state
                if hasattr(state, 'user_data') and state.user_data.get('age'):
                    try:
                        age = int(state.user_data['age'])
                    except Exception:
                        age = None
                # If age is present and under 18, block and show main menu button
                if age is not None and age < 18:
                    state.current_step = "complete"
                    return self._create_button_response(
                        content="Sorry, Satu Gaji Satu Harapan is only available for users aged 18 and above.\nYou cannot continue with this campaign.",
                        buttons=[
                            {"label": "🏠 Return to Main Menu", "value": "main_menu"}
                        ],
                        next_step="complete",
                ...
                # Otherwise, proceed to premium estimation
        ...
        try:
            ...
            if 'dob' in state.user_data and state.user_data['dob']:
                try:
                    ...
                    if age < 18:
                        error_msg = (
                            "Sorry, this campaign is only available for users aged 18 and above.\n"
                            "You cannot continue with this campaign."
                        )
                        # Prevent further progress in this campaign
                        state.current_step = "complete"
                        return self._create_button_response(
                            content=error_msg,
                            buttons=[
            ...
            try:
                ...
                if age < 18 or age > 65:
                    if age < 18:
                        ...
                        error_msg = "Please enter an age between 18 and 65."
                        if ws:
                            await ws.send_text(json.dumps({
                                "type": "message",
                                "content": error_msg,
    ...
    async def _handle_agent_decision(self, user_id: str, message: Union[str, dict], state: Dict[str, Any], ws=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if message_lower in ['yes', 'y', 'ya', 'yeah', 'yes_contact', 'contact_agent']:
                contact_status = "Yes, Contact Requested"
            elif message_lower in ['no', 'n', 'no_contact', 'no thanks']:
                contact_status = "No, Contact Declined"
            ...
            if contact_status is not None:
                try:
                    ...
                    annual_income_str = str(state.user_data.get("annual_income",""))
                    coverage_str = str(state.user_data.get("years_of_coverage",""))
                    ...
                    row_data = [
                        ...
                        annual_income_str, coverage_str,None,None,None,None,None,contact_status # New: annual_income, coverage, monthly_premiu
                          ]
                ...
                return self._create_button_response(
                    content="Thank you for your interest in Satu Gaji Satu Harapan! If you wish to return to the main menu, click below.",
                    buttons=[
                        {"label": "🏠 Return to Main Menu", "value": "main_menu"}
                    ],
                    next_step="handle_agent_decision",
