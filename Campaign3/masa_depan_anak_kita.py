...
class MasaDepanAnakKita:
    ...
    def _get_user_age_from_userdata(self, user_data: Dict[str, Any]) -> Optional[int]:
        """Try to determine user's age from user_data.
        Supports 'age' (int) or 'dob' strings in common formats (YYYY-MM-DD, DD/MM/YYYY, DD-MM-YYYY).
        Returns age in years or None if unknown/parsable.
        """
        if not user_data:
            return None
        ...
        # try dob field
        dob = user_data.get("dob") or user_data.get("date_of_birth")
        ...
        # fallback: try extract year from digits (e.g., 1985)
        years = re.findall(r'\d{4}', dob)
        if years:
            try:
                yr = int(years[0])
                birth = datetime(yr, 1, 1)
                age_calc = today.year - birth.year
                return int(age_calc)
            except Exception:
                pass
        return None

    async def process_message(self, user_id: str, message: Union[str, dict], ws: Any = None,
        ...
        try:
            ...
            help_response = await self._handle_help_request(message_content, state, ws)
            if help_response:
            ...
            elif state.current_step in ["ask_about_estimation", "waiting_for_estimation_response"]:
                logger.info(f"[MDK] Processing estimation response: {message}")

                # --- NEW: detect user's own age and restrict if under 18 ---
                user_age = self._get_user_age_from_userdata(state.user_data)
                if user_age is not None and user_age < 18:
                    state.current_step = "end_options"
                    return {
                        "type": "buttons",
                        "content": "Sorry, Masa Depan Anak Kita is only available for users aged 18 and above.\nYou cannot continue with this campaign.",
                        "campaign_data": state.user_data,
                        "next_step": "end_options",
                        "buttons": [
                            {"label": "🏠 Return to Main Menu", "value": "main_menu"}
            ...
            # Handle monthly saving amount selection
                ...
                try:
                    ...
                    # Check if user is responding to the saving options with yes/no
                    ...
                    if amount is None:
                        numbers = re.findall(r'\d+', message)
                        if numbers:
                            amount_candidate = int(numbers[0])
                            if amount_candidate in [200, 300, 400, 500]:
            ...
            # Handle custom saving amount input
                ...
                try:
                    ...
                    result_message = (
                        ...
                        f"{'='*10}\n"
                        f"The amount stated are estimations and subject to fund performance.\n"
            ...
            # Handle completion and restart
                ...
                if message.lower() in ["contact_agent", "no_contact", "no"]:
                    # Update Google Sheet
                    try:
                        ...
                        child_age_str = str(state.child_age) if state.child_age is not None else ""
                        monthly_saving_str = str(state.monthly_saving) if state.monthly_saving is not None else ""

                        contact_status = (
                       "Yes, Contact Requested" if message.lower() == "contact_agent" else "No, Contact Declined")
                        
                        row_data = [
                            name, dob, email, primary_concern, life_stage, dependents,
                            existing_coverage, premium_budget, selected_plan,
                            None, None, None,
                            child_age_str, monthly_saving_str, None, None, contact_status
                        ]
                    ...
                    return {
                        "type": "buttons",
                        "content": "Thank you for your interest in Masa Depan Anak Kita! If you wish to return to the main menu, click below.",
                        "campaign_data": state.user_data,
    ...
    async def calculate_results(self, message: str, state: CampaignState) -> Dict[str, Any]:
        ...
        try:
            ...
            result_message = (
                ...
                f"{format_currency(fv_low)} and {format_currency(fv_high)} by the time your child "
                "reaches university age (18).\n\n"
                "REMINDER:The amount stated are estimations and subject to fund performance. "
            )
...
async def main(ws, state):
    ...
    """
    # Use WebSocket ID as user ID
    user_id = id(ws)

    # Start the campaign flow and return the response
    response = await masa_depan_anak_kita_campaign.start(ws, user_id)
    return response
