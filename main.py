    ...
    while conversation_active:
        try:
            ...
            if not state.active_campaign and state.step == "get_name":
                ...
                await send_question(ws, f"Thank you, {name}! When were you born? (DD/MM/YYYY)")
                state.step = "get_dob"
                ...
                if not re.match(r'^\d{2}/\d{2}/\d{4}$', dob):
                    ...
                    try:
                        ...
                        # Ask for email
                        await send_question(ws, f"Finally, please share your email address: \n(your.email@example.com)\n please include a right email so it could be easy to contact you")
                ...
                primary_concern = message_content.strip()
                state.user_data["primary_concern"] = primary_concern
    
                filler_message ={
                    "income_protection":"Great, you want to protect your family's income. That shows you're thinking ahead about their stability.",
                    "medical_expenses":"Great, you want to cover medical expenses. Health is truly the most important asset.",
                    "education":"Great, you want to fund your children's education. That's an investment in their future",
                    "wealth_building": "Great, you want to build long-term wealth.That's a wise step for financial independence.",
                    "retirement":"Great, you want to plan for retirement.Preparing early makes life easier later."
                }
                if primary_concern in filler_message:
                    await send_text(ws, filler_message[primary_concern])
                ...
                dependents_filler_message ={
                    "1": "Okay, just yourself -we'll focus on personal protection and growth.",
                    "2":"Thank you, having one dependents means securing their future is important.",
                    "3": "Great, with 2-3 dependents, family protection becomes even more critical.",
                    "4": "Understood, with 4 or more dependets, your plan must be strong and comprehensive."
                }
                selected_dependents = message_content.strip()
                if selected_dependents in dependents_filler_message:
                    await send_text(ws, dependents_filler_message[selected_dependents])
                ...
                existing_coverage_filler ={
                    "none":"Noted, no coverage yet - this is the right time to start.",
                    "basic":"Okay, you have basic coverage - let's see how we can add more protection.",
                    "some":"Good, you already have some protection - let's strengthen it further.",
                    "full":"Great, You already have strong coverage - let's review and enhance it if needed."
                }
                selected_coverage = message_content.strip()
                if selected_coverage in existing_coverage_filler:
                    await send_text(ws, existing_coverage_filler[selected_coverage])
